...
**Description:** Genişletilmiş production fingerprint alındı: `production`, `app_debug=false`, PHP `8.3.33`, Laravel `12.67.0`, MariaDB `10.3.39`, `REPEATABLE-READ`, database queue, wallet/claim tabloları, migration batch `100`, config/routes cache ve release SHA mevcut. Queue pending `0`, worker heartbeat `2s`, cron heartbeat `5s`; `failed_jobs=0` tinker ve queue komutuyla doğrulandı. Reverse-proxy route’ları ayrıca doğrulanmayı bekliyor.
...
**Recommended fix:** Reverse-proxy route kanıtını ayrıca kaydet ve yalnız güvenli smoke test çalıştır.
