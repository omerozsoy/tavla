...
**Evidence:** Runtime ekonomik yolları WalletService çağırıyor; `ResetCoins` komutu da artık kullanıcı başına kilitli WalletService + ledger yazımı kullanıyor. WalletService ayrıca kilitli güncel bakiyeyi çağıran model nesnesine senkronlayarak sonraki stale `save()` çağrısının hareketi geri yazmasını engelliyor. Kod taramasında yalnızca E2E seed fixture'ında doğrudan başlangıç bakiyesi ataması kaldı; production migration ve gerçek queue retry kanıtı yok. Wallet/product/reward atomiklik testleri **25 test / 185 assertions** geçti.  
