 ...
 *   tier        bronze|silver|gold|diamond|null   (SADECE coin odulu icin; rarity'den bagimsiz)
 *   rarity      ZORLUK sinifi (common..mythic) — rozetin ELDE ETME zorlugunu yansitir.
 *               Kolaydan zora: common < uncommon < rare < epic < legendary < mythic.
 *               (Eskiden canli unlock oranindan turetiliyordu; kucuk kullanici tabaninda
 *                her sey 'mythic' cikiyordu — artik SABIT zorluk sinifi.)
 ...
 *
 * NOT: Gercek unlock orani ('rarityPct') bilgi amacli ayrica dondurulur ("oyuncularin %X'i
 * kazandi"); ama gorunen rarity SINIFI artik bu SABIT zorluga gore belirlenir.
 */

// ---- Kademe (tier) coin odulleri (rarity'den BAGIMSIZ; sadece odul olcegi) ----
$TIER = [
    'bronze'  => ['coin' => 15],
    'silver'  => ['coin' => 30],
    'gold'    => ['coin' => 50],
 ...
 * $steps: [ [value, tier, rarity, name, desc], ... ]
 *   tier   -> coin odulu (bronze/silver/gold/diamond)
 *   rarity -> ZORLUK sinifi (common..mythic), esige gore ELLE belirlenir.
 */
$fam = function (string $prefix, string $category, string $icon, string $metric, array $steps) use ($TIER): array {
    $out = [];
    foreach ($steps as $i => [$value, $tier, $rarity, $name, $desc]) {
        $out[] = [
            'slug' => $prefix.'_'.($i + 1),
            'category' => $category,
            'name' => $name,
...
// ============ MAÇ SAYISI (saf zaman/grind) ============
$list = array_merge($list, $fam('match', 'match', 'dice', 'total_matches', [
    [10,    'bronze',  'common',    'Çaylak',          '10 maç oyna.'],
    [50,    'bronze',  'common',    'Hevesli',         '50 maç oyna.'],
    [100,   'silver',  'uncommon',  'Müdavim',         '100 maç oyna.'],
    [500,   'silver',  'uncommon',  'Tahta Dostu',     '500 maç oyna.'],
    [1000,  'gold',    'rare',      'Tahta Eskisi',    '1.000 maç oyna.'],
    [2500,  'gold',    'rare',      'Kronikleşti',     '2.500 maç oyna.'],
    [5000,  'diamond', 'epic',      'Yaşayan Efsane',  '5.000 maç oyna.'],
    [10000, 'diamond', 'legendary', 'Tavla Bağımlısı', '10.000 maç oyna.'],
]));

// ============ GALİBİYET ============
$list = array_merge($list, $fam('wins', 'wins', 'trophy', 'total_wins', [
    [1,     'bronze',  'common',    'İlk Galibiyet',  'İlk maçını kazan.'],
    [10,    'bronze',  'common',    'İlk Zafer',      '10 galibiyet elde et.'],
    [50,    'silver',  'uncommon',  'Yükseliş',       '50 galibiyet elde et.'],
    [100,   'silver',  'uncommon',  'Kazanan',        '100 galibiyet elde et.'],
    [500,   'gold',    'rare',      'Usta Kazanan',   '500 galibiyet elde et.'],
    [1000,  'gold',    'epic',      'Tavla Canavarı', '1.000 galibiyet elde et.'],
    [5000,  'diamond', 'legendary', 'Yenilmez',       '5.000 galibiyet elde et.'],
    [10000, 'diamond', 'mythic',    'Ölümsüz',        '10.000 galibiyet elde et.'],
]));

// ============ GALİBİYET SERİSİ (beceri + sans; ust seviye cok zor) ============
$list = array_merge($list, $fam('streak', 'streak', 'flame', 'best_win_streak', [
    [3,  'bronze',  'common',    'Seri Başladı',   'Üst üste 3 maç kazan.'],
    [5,  'bronze',  'uncommon',  'Alev Aldı',      'Üst üste 5 maç kazan.'],
    [10, 'silver',  'rare',      'Durdurulamıyor', 'Üst üste 10 maç kazan.'],
    [20, 'gold',    'epic',      'Kim Tutar Seni?','Üst üste 20 maç kazan.'],
    [30, 'gold',    'legendary', 'Makine',         'Üst üste 30 maç kazan.'],
    [50, 'diamond', 'mythic',    'Dokunulmaz',     'Üst üste 50 maç kazan.'],
]));

// ============ TAVLA / MARS ============
$list = array_merge($list, $fam('gammon', 'tavla', 'target', 'total_gammons', [
    [1,    'bronze',  'common',    'Marslı',         'İlk kez rakibini marsla (gammon) yen.'],
    [10,   'silver',  'uncommon',  'Mars Avcısı',    '10 mars galibiyeti elde et.'],
    [100,  'gold',    'rare',      'Mars Fatihi',    '100 mars galibiyeti elde et.'],
    [1000, 'diamond', 'legendary', 'Mars İmparatoru','1.000 mars galibiyeti elde et.'],
]));
$list = array_merge($list, $fam('backgammon', 'tavla', 'target', 'total_backgammons', [
    [1,   'silver',  'uncommon', 'Katmerli', 'İlk kez katmerli marsla (backgammon) yen.'],
    [10,  'gold',    'rare',     'Acımasız', '10 katmerli mars galibiyeti elde et.'],
    [100, 'diamond', 'epic',     'İnfazcı',  '100 katmerli mars galibiyeti elde et.'],
]));
$list[] = $ev('tavla_katmerli_win', 'tavla', 'Acımadı', 'Rakibini katmerli marsla yen.', 'target', 'flag_backgammon_win', 'uncommon', 250);
...
$list = array_merge($list, $fam('doubles', 'dice', 'dice', 'total_doubles', [
    [10,   'bronze',  'common',   'Zar Isındı',       '10 çift zar at.'],
    [100,  'silver',  'uncommon', 'Dubleci',          '100 çift zar at.'],
    [1000, 'gold',    'rare',     'Zarların Efendisi','1.000 çift zar at.'],
    [5000, 'diamond', 'epic',     'Zar Kâhini',       '5.000 çift zar at.'],
]));

// ============ AI ANALİZ / YETENEK ============
$list = array_merge($list, $fam('analysis', 'analysis', 'star', 'analysis_count', [
    [1,   'bronze',  'common',   'Öğrenci',         'İlk maç analizini yap.'],
    [10,  'bronze',  'common',   'Dikkatli Oyuncu', '10 maç analizi biriktir.'],
    [50,  'silver',  'uncommon', 'Meraklı',         '50 maç analizi biriktir.'],
    [100, 'gold',    'rare',     'Analiz Delisi',   '100 maç analizi biriktir.'],
    [500, 'diamond', 'epic',     'Kütüphane',       '500 maç analizi biriktir.'],
]));
$list = array_merge($list, $fam('clean', 'analysis', 'check', 'clean_matches', [
    [1,   'bronze',  'uncommon',  'Hatasız Gece', 'Bir maçı hiç blunder yapmadan bitir.'],
    [10,  'silver',  'rare',      'İstikrar',     '10 hatasız (blunder’sız) maç oyna.'],
    [50,  'gold',    'epic',      'Temiz Sicil',  '50 hatasız maç oyna.'],
    [100, 'diamond', 'legendary', 'Profesör',     '100 hatasız maç oyna.'],
]));
...
// ============ KÜP ============
$list[] = $ev('cube_first_double', 'cube', 'Cesur', 'İlk doğru Double (katlama) kararını ver.', 'target', 'flag_correct_double', 'common', 150);
$list[] = $ev('cube_first_take', 'cube', 'Yemezler', 'İlk doğru Take (kabul) kararını ver.', 'target', 'flag_correct_take', 'common', 150);
$list[] = $ev('cube_hard_take', 'cube', 'Soğukkanlı', 'Zor bir Take kararını doğru ver.', 'target', 'flag_hard_take', 'rare', 400);
$list[] = $ev('cube_take_win', 'cube', 'Küpü Ver!', 'Rakibin Double teklifini Take edip maçı kazan.', 'trophy', 'flag_take_and_win', 'uncommon', 250);
$list = array_merge($list, $fam('cubes', 'cube', 'target', 'correct_cube_decisions', [
    [10,   'bronze',  'common',   'Küp Acemisi',   '10 doğru küp kararı ver.'],
    [100,  'silver',  'uncommon', 'Küp Ustası',    '100 doğru küp kararı ver.'],
    [500,  'gold',    'rare',     'Küp Uzmanı',    '500 doğru küp kararı ver.'],
    [1000, 'diamond', 'epic',     'Küp Profesörü', '1.000 doğru küp kararı ver.'],
]));

// ============ COIN / SERVET (anlık bakiye — ekonomi grind) ============
$list = array_merge($list, $fam('coin', 'coin', 'gift', 'coin_balance', [
    [1000,     'bronze',  'common',    'İlk Maaş',        '1.000 coin biriktir.'],
    [5000,     'bronze',  'common',    'Cebi Para Gördü', '5.000 coin biriktir.'],
    [10000,    'silver',  'uncommon',  'Para Babası',     '10.000 coin biriktir.'],
    [50000,    'silver',  'uncommon',  'Kasa Sağlam',     '50.000 coin biriktir.'],
    [100000,   'gold',    'rare',      'Tavla Baronu',    '100.000 coin biriktir.'],
    [1000000,  'gold',    'epic',      'Coin Milyoneri',  '1.000.000 coin biriktir.'],
    [5000000,  'diamond', 'legendary', 'Darphane',        '5.000.000 coin biriktir.'],
    [10000000, 'diamond', 'mythic',    'Merkez Bankası',  '10.000.000 coin biriktir.'],
]));
// LIFETIME kazanılan coin (ileri-dönük izlenir).
$list = array_merge($list, $fam('earned', 'coin', 'gift', 'lifetime_coin', [
    [10000,   'silver',  'uncommon', 'Emekçi',          'Toplam 10.000 coin kazan.'],
    [100000,  'gold',    'rare',     'Kazanç Makinesi', 'Toplam 100.000 coin kazan.'],
]));

// ============ TURNUVA ============
$list = array_merge($list, $fam('tourney', 'tournament', 'trophy', 'tournaments_won', [
    [1,  'bronze',  'uncommon',  'Şampiyon',            'Bir turnuva kazan.'],
    [5,  'silver',  'rare',      'Kupa Avcısı',         '5 turnuva kazan.'],
    [10, 'gold',    'epic',      'Kupa Koleksiyoncusu', '10 turnuva kazan.'],
    [50, 'diamond', 'legendary', 'Turnuva Efsanesi',    '50 turnuva kazan.'],
]));
$list = array_merge($list, $fam('tourneyplay', 'tournament', 'trophy', 'tournaments_played', [
    [1,  'bronze', 'common',   'Arenaya Adım',   'İlk turnuvana katıl.'],
    [10, 'silver', 'uncommon', 'Turnuva Kurdu',  '10 turnuvaya katıl.'],
    [50, 'gold',   'rare',     'Sürekli Rakip',  '50 turnuvaya katıl.'],
]));
...
$list = array_merge($list, $fam('peak', 'rating', 'crown', 'best_rating', [
    [1600, 'silver',  'uncommon',  'Usta',        '1600 rating’e ulaş.'],
    [1800, 'gold',    'rare',      'Master',      '1800 rating’e ulaş.'],
    [2000, 'diamond', 'epic',      'Grandmaster', '2000 rating’e ulaş.'],
    [2200, 'diamond', 'legendary', 'Efsane',      '2200 rating’e ulaş.'],
]));
...
$list[] = $ev('lb_top50', 'rating', 'İlk 50', 'Liderlik tablosunda ilk 50’ye gir.', 'crown', 'flag_top_50', 'epic', 750);
$list[] = $ev('lb_top10', 'rating', 'Elitler Kulübü', 'Liderlik tablosunda ilk 10’a gir.', 'crown', 'flag_top_10', 'legendary', 1500);
$list[] = $ev('lb_top1', 'rating', 'Zirvede', 'Liderlik tablosunda 1. sıraya çık.', 'crown', 'flag_top_1', 'mythic', 3000);
...
$list = array_merge($list, $fam('grind', 'fun', 'dice', 'matches_today', [
    [20, 'silver', 'uncommon', 'Bir El Daha',    'Tek günde 20 maç oyna.'],
    [50, 'gold',   'rare',     'Mesaiye Kaldık', 'Tek günde 50 maç oyna.'],
]));
...
return [
    // Rarity BANTLARI artik gorunen rarity'yi BELIRLEMEZ (rarity = sabit zorluk sinifi, bkz
    // yukarida). Bu bant tablosu yalnizca "oyuncularin %X'i kazandi" gibi ISTATISTIK
    // gostermek isteyen yerler icin referans olarak korunur.
    'rarity_bands' => [
        'common'    => 0.40,
        'uncommon'  => 0.20,
        'rare'      => 0.05,
