...
- **Adım E — Frontend authoritative tamamla** ✅ KOD TAMAM (commit 9f78ca8, DORMANT): api.ts serverCubeOffer/Respond/Resign + ServerMatch tipi; `applyServerBoard` tam senkron (tahta+skor+küp+cubePending+yeni-oyun açılış tetikleme); doRollAuthoritative açılış (starter+zar); opening useEffect authoritative→serverRoll; küp/resign handler'ları authoritative→sunucu; bear-off yerel skorlama authoritative'de atlanır. tsc+99 frontend+181 backend yeşil. **DOĞRULANMADI** (2-istemci staging şart — Adım G).
